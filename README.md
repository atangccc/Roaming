...
- 使用 `/`、`/archives`、`/tags`、文章永久链接等真实站点路径切换主题。
- 访客选择使用独立 Cookie 保存，支持会话、7 天或 30 天。
- 失效、未就绪或被关闭的主题会在服务端自动拒绝并安全回退。
- 前台入口由插件自动注入，并使用 Shadow DOM 隔离主题样式。
- 为主题作者提供声明式锚点、JavaScript API、公共目录接口及 Thymeleaf Finder。
